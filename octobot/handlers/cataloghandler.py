...
def create_inline_buttons(command, query, current_index, max_results, previous_offset, next_offset):
    return [
        ...
        telegram.InlineKeyboardButton(
            text=f"{current_index}/{max_results}", callback_data="nothing:"),
...
class CatalogHandler(octobot.filters.CommandFilter):
    ...
    def handle_page(self, bot, context: Context):
        ...
        offset = offset
        try:
            res: octobot.Catalog = self.function(
                query, offset, 1, bot, context)
    ...
    def handle_inline(self, bot, context: Context):
        ...
        for item in res:
            ...
            item.reply_markup.inline_keyboard.append(
                self.create_current_query_button(query, context))
