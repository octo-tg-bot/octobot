...
def get_apex_stats(platform, username, context):
    if good_username(username):
        ...
        player_info.append(context.localize("Current legend: <b>{}</b>").format(
            CHARACTER_EMOJIS.get(selected_legend_name.lower(), "") + context.localize(selected_legend_name),
        ...
        for legend_name, legend in res["legends"]["all"].items():
            text = [f"{CHARACTER_EMOJIS.get(legend_name.lower(), '')}{context.localize(legend_name)}"]
            ...
            button = telegram.InlineKeyboardButton(
                text=CHARACTER_EMOJIS.get(legend_name.lower(), '') + context.localize(legend_name),
