...
def help_command(query, idx, max_amount, bot: octobot.OctoBot, ctx: octobot.Context):
    ...
    for handler in handlers[idx]:
        if last_plugin != handler.plugin["friendly_name"]:
            last_plugin = ctx.localize(handler.plugin["friendly_name"])
            handlers_msg += f"<b>{handler.plugin['friendly_name']}:</b>\n"
